...
function RpcClient({ host = '127.0.0.1', port = 65100, user = 'user', pass = 'pass', protocol = 'http', rejectUnauthorized = false }) {
    this.host = host
    this.port = port
    this.user = user
    this.pass = pass
    ...
    this.rejectUnauthorized = rejectUnauthorized//isRejectUnauthorized ? opts.rejectUnauthorized : true
...
function innerRpc(request, callback) {
    ...
    const errorMessage = 'Wall JSON-RPC: '
    ...
    const req = this.protocol.request(options, (res) => {
        ...
        res.on('end', () => {
            ...
            if (res.statusCode === 401) {
                callback(`${errorMessage}Connection Rejected: 401 Unnauthorized`)
                return
            }
            if (res.statusCode === 403) {
                callback(`${errorMessage}Connection Rejected: 403 Forbidden`)
                return
            }
            if (res.statusCode === 403) {
                callback(`${errorMessage}Connection Rejected: 403 Forbidden`)
            ...
            if (res.statusCode === 500 && buf.toString('utf8') === 'Work queue depth exceeded') {
                const exceededError = `${errorMessage}${buf.toString('utf8')}`
            ...
            try {
                ...
                const err = `${errorMessage}Error Parsing JSON: ${e.message}`
                callback(err)
                return
            }
    ...
    req.on('error', (e) => {
        if (!called) {
            called = true
            callback(`${errorMessage}Request Error: ${e.message}`)
        }
    })
